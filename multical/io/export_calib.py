...
def export_relative(camera_names, camera_poses, master):
    ...
    return {
        k if master == k else f"{k}_to_{master}": export_transform(pose)
        for k, pose, valid in zip(camera_names, camera_poses.poses, camera_poses.valid)
        if valid
    }
...
def export_poses(pose_table, names=None):
    ...
    return {
        i: t.poses.tolist() for i, t in zip(names, pose_table._sequence()) if t.valid
    }
...
def export_images(camera_names, filenames):
    return struct(
        rgb=[
            {camera: image for image, camera in zip(images, camera_names)}
...
def export_json(calib, names, filenames, master=None):
    ...
    data = struct(
        ...
        camera_poses=(
            export_camera_poses(names.camera, camera_poses)
            if master is None
            else export_relative(names.camera, camera_poses, master)
        ),
