...
def hand_eye_robot_world(world_wrt_camera, base_wrt_gripper):
    ...
    err = matrix.transform(base_wrt_world, world_wrt_camera) - matrix.transform(
        base_wrt_gripper, gripper_wrt_camera
    )
...
def hand_eye(world_wrt_camera, base_wrt_gripper):
    ...
    camera_gripper_r, camera_gripper_t = cv2.calibrateHandEye(
        base_gripper_r, base_gripper_t, world_camera_r, world_camera_t
    )
    ...
    base_wrt_world = matrix.mean_robust(
        matrix.transform(
