...
def show_boards(args):
    ...
    assert (
        args.paper_size_mm is None or args.paper_size is None
    ), "specify --paper_size_mm or --paper_size (not both)"
    ...
    if args.paper_size is not None:
        ...
        assert (
            len(paper_size_mm) == 2
        ), f"expected WxH paper_size_mm e.g. 420x594 or name, one of {list(standard_sizes.keys())}"
    ...
    def draw_board(board):
        ...
        if paper_size_mm is not None:
            ...
            image[dy : dy + board_image.shape[0], dx : dx + board_image.shape[1]] = (
                board_image
            )
            return image
