...
def transformed_linear(self, camera_poses, world_points, times):
    ...
    def transform(time_poses):
        pose_table = tables.expand_views(struct(camera=camera_poses, times=time_poses))
...
def transformed_interpolate(self, camera_poses, world_points, times):
    ...
    valid = (
        np.expand_dims(camera_poses.valid, [1, 2, 3])
    ...
    return struct(
        points=matrix.transform_homog(
            t=view_poses, points=np.expand_dims(world_points.points, (0, 1))
        ),
        valid=valid,
    )
...
class RollingFrames(MotionModel, Parameters):
    ...
    def pre_transform(self, t):
        return self.copy(pose_start=t @ self.pose_start, pose_end=t @ self.pose_end)

    def post_transform(self, t):
        return self.copy(pose_start=self.pose_start @ t, pose_end=self.pose_end @ t)
    ...
    def _project(self, cameras, camera_poses, world_points, estimates=None):
        ...
        times = (
            rolling_times(cameras, estimates)
    ...
    def sparsity(self, index_mapper: IndexMapper, axis: int):
        start, end = [
            index_mapper.pose_mapping(t, axis=axis, param_size=6)
    ...
    def export(self):
        return {
            i: struct(start=start.tolist(), end=end.tolist())
            for i, start, end, valid in zip(
                self.names, self.pose_start, self.pose_end, self.valid
            )
            if valid
        }
