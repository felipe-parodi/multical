...
class HandEye:
    ...
    def check_viewed_boards(self):
        ...
        for idx, cam in enumerate(self.cam_names):
            boards[cam] = [
                b
                for b in range(0, self.num_boards)
                if self.pose_table.valid[idx][:, b].sum() > limit_board_image
            ]
        return boards
    ...
    def initialise_camera_poses(self):
                ...
                if slave_cam != master_cam:
                    ...
                    for boardM in self.viewed_boards[master_cam]:
                        for boardS in self.viewed_boards[slave_cam]:
                            slaveCam_wrt_masterCam, image_ids = self.master_slave_pair(
                                idm, ids, boardM, boardS
                            )
                            if slaveCam_wrt_masterCam is not None:
                                self.handeye_df.append(
                                    {
                                        ...
                                        "boardS": boardS,
                                        "image_ids": image_ids,
                                        "slaveCam_wrt_masterCam": slaveCam_wrt_masterCam,
                                    }
                                )
                                cs_wrto_cm.append(slaveCam_wrt_masterCam)
                    self.camera_groups[master_cam][slave_cam] = [
                        g.tolist() for g in cs_wrto_cm
                    ]
                    density, self.camera_poses[master_cam][slave_cam] = (
                        probabilistic_guess(cs_wrto_cm)
                    )
        ...
        info(
            f"Reference camera {self.reference_camera} with density {max_density} and number of groups {max_group}"
        )
        init0 = relative_to_cam(
            self.cam_names[0], self.camera_poses[self.reference_camera]
        )
    ...
    def master_slave_pair(self, master_cam, slave_cam, boardM, boardS):
        ...
        """
        table_m = self.pose_table._index_select(master_cam, axis=0)._index_select(
            boardM, axis=1
        )
        table_s = self.pose_table._index_select(slave_cam, axis=0)._index_select(
            boardS, axis=1
        )
        ...
        (slaveCam_wrt_masterCam, slaveB_wrt_masterB) = self.hand_eye_robot_world(
            master_R, master_t, slave_R, slave_t
        )
    ...
    def hand_eye_robot_world(cam_world_R, cam_world_t, base_gripper_R, base_gripper_t):
        try:
            ...
            gripper_wrt_world = matrix.join(
                gripper_world_r, gripper_world_t.reshape(-1)
            )
