...
class ParamList(Parameters, Generic[T]):
    ...
    def with_params(self, params):
        ...
        updated = [obj.with_param_vec(p) for obj, p in zip(self.param_objects, params)]
...
class IndexMapper(object):
    ...
    def __init__(self, valid_mask):
        self.mask_coords = np.broadcast_to(
            np.expand_dims(valid_mask, -1), [*valid_mask.shape, 2]
        )
    ...
    def param_indexes(self, params, axis):
        return [
            (p.size, self.point_indexes(i, axis=axis)) for i, p in enumerate(params)
        ]

    def pose_mapping(self, poses, axis, param_size):
        return [
            (param_size, self.point_indexes(i, axis, optimized))
