...
default_optimize = struct(
    cameras=False, boards=False, camera_poses=True, board_poses=True, motion=True
)
...
class Calibration(parameters.Parameters):
    def __init__(
        self,
        cameras: ParamList[Camera],
        boards: ParamList[Board],
        point_table: Table,
        camera_poses: PoseSet,
        board_poses: PoseSet,
        motion: MotionModel,
        inlier_mask=None,
    ...
    def size(self):
        ...
        return struct(
            cameras=cameras, rig_poses=rig_poses, boards=boards, points=points
        )
    ...
    def valid(self):
        ...
        valid = (
            np.expand_dims(self.camera_poses.valid, [1, 2])
    ...
    def world_points(self):
        return tables.transform_points(
            tables.expand_dims(self.board_poses.pose_table, 1), self.board_points
        )

    @cached_property
    def pose_estimates(self):
        return struct(
            camera=self.camera_poses.pose_table,
    ...
    def projected(self):
        ...
        return self.motion.project(
            self.cameras, self.camera_poses.pose_table, self.world_points
        )
    ...
    def with_params(self, params):
        ...
        updated = {
            k: self.param_objects[k].with_param_vec(param_vec)
    ...
    def sparsity_matrix(self):
        ...
        mapping_list = [
            mapping for k, mapping in param_mappings.items() if self.optimize[k] is True
        ]

        return parameters.build_sparse(sum(mapping_list, []), mapper)

    def bundle_adjust(
        self, tolerance=1e-4, f_scale=1.0, max_iterations=100, loss="linear"
    ):
        ...
        def evaluate(param_vec):
            ...
            return (calib.reprojected.points - calib.point_table.points)[
                self.inliers
            ].ravel()
        ...
        with contextlib.redirect_stdout(LogWriter.info()):
            res = optimize.least_squares(
                evaluate,
                self.param_vec,
                jac_sparsity=self.sparsity_matrix,
                verbose=2,
                x_scale="jac",
                f_scale=f_scale,
                ftol=tolerance,
    ...
    def reject_outliers(self, threshold):
        ...
        info(
            f"Rejecting {num_outliers} outliers with error > {threshold:.2f} pixels, "
    ...
    def adjust_outliers(
        self, num_adjustments=3, select_scale=None, select_outliers=None, **kwargs
    ):
        info(
            f"Beginning adjustments ({num_adjustments}) enabled: {self.optimize}, options: {kwargs}"
        )
    ...
    def plot_errors(self):
        ...
        axs[0].scatter(
            x=np.arange(errors.size)[inliers],
            y=errors[inliers],
            marker=".",
            label="inlier",
        )
        axs[0].scatter(
            x=np.arange(errors.size)[outliers],
            y=errors[outliers],
            color="r",
            marker=".",
            label="outlier",
        ...
        axs[1].hist(
            errors[valid], bins=50, range=(0, np.quantile(errors[valid], 0.999))
        )
    ...
    def report(self, stage=""):
        ...
        if self.inlier_mask is not None:
            info(
                f"{stage} reprojection RMS={inliers.rms:.3f}, n={inliers.n}, quantiles={inliers.quantiles}"
            )
        else:
            overall = error_stats(self.reprojection_error)
            info(
                f"{stage} reprojection RMS={overall.rms:.3f}, n={overall.n}, quantiles={overall.quantiles}"
            )
