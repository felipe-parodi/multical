...
def projection_corners(camera, scale=1):
    return np.concatenate([np.zeros((1, 3)), view_projection(camera, size=scale)])
...
class SceneMeshes:
    def __init__(self, calib, camera_scale=1):
        self.board = [
            pv.PolyData(board.mesh.points, board.mesh.polygons)
        ...
        self.camera_projections = [
            projection_corners(camera, scale=1) for camera in calib.cameras
        ]
        self.camera = [
            pv.PolyData(self.camera_scale * proj, view_triangles)
    ...
    def update(self, calib):
        self.camera_projections = [
            projection_corners(camera) for camera in calib.cameras
        ]
...
camera_colors = struct(
    inactive=(0.5, 0.5, 0.5), active_camera=(1.0, 1.0, 0.0), active_set=(0.0, 1.0, 0.0)
)
...
class CameraSet:
    def __init__(self, viewer, camera_poses, camera_meshes):
        ...
        self.instances = [
            Marker(viewer, mesh, pose, options)
    ...
    def update(self, highlight, active=True):
        for i, marker in enumerate(self.instances):
            color = (
                camera_colors.inactive
                if not active
                else (
                    camera_colors.active_camera
                    if i == highlight
...
class BoardSet:
    def __init__(self, viewer, board_poses, board_meshes, board_colors):
        ...
        self.instances = [
            instance(mesh, pose, color)
            for mesh, color, pose in zip(
                board_meshes, board_colors, board_poses._sequence()
            )
        ]
