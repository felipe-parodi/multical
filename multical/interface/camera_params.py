...
class ParamsViewer(QtWidgets.QScrollArea):
    ...
    def update_cameras(self):
        ...
        for camera_widget, camera, camera_pose in zip(
            self.camera_widgets, self.cameras, poses.poses
        ):
    ...
    def set_camera(self, camera):
        ...
        for i in range(3):
            for j in range(3):
                ...
                self.intrinsic_table.setItem(
                    i, j, QtWidgets.QTableWidgetItem(f"{v:.4f}")
                )
