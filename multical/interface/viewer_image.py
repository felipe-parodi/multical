...
def qt_image(image):
    ...
    if image.ndim == 3:
        ...
        bytesPerLine = width
        return QImage(
            image.data, width, height, bytesPerLine, QImage.Format.Format_Grayscale8
        )
...
def cross(point, radius, pen):
    x, y = point
    item = group(
        [line([-radius, 0], [radius, 0], pen), line([0, -radius], [0, radius], pen)]
    )
    item.setPos(x, y)
    return item
...
colors = struct(
    error_line=(1, 1, 0), outlier=(1, 0, 0), inlier=(0, 1, 0), invalid=(0.5, 0.5, 0)
)
...
def add_reprojections(scene, points, projected, inliers, boards, valid_boards, options):
    ...
    for board, valid_board, board_points in zip(
        boards, valid_boards, frame_table._sequence(0)
    ):
        if not valid_board:
            continue
        ...
        for point, id in zip(board_points._sequence(), board.ids):
            ...
            color_key = (
                "invalid"
                if not point.valid
                else ("inlier" if point.inlier else "outlier")
            )
            add_marker(
                scene,
                point.proj,
...
def annotate_image(workspace, calibration, layer, state, options):
    ...
    if layer == "detections":
        ...
        for board, color, points in zip(
            workspace.boards, workspace.board_colors, detections._sequence(0)
        ):
        ...
        for board, pose, color in zip(
            workspace.boards, board_poses._sequence(0), workspace.board_colors
        ):
            if pose.valid:
