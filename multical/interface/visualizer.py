...
class Visualizer(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        toolbar_combo = widget(
            h_layout(QtWidgets.QLabel("Stage"), self.calibrations_combo), self
        )
    ...
    def setup_view_table(self, calibration):
        ...
        with self.updating:
            ...
            self.view_model = (
                view_table.ViewModelDetections(ws.point_table, ws.names)
                if calibration is None
                else view_table.ViewModelCalibrated(calibration, ws.names)
            )
    ...
    def update_image(self):
        ...
        annotated_image = annotate_image(
            self.workspace, self.calibration, layer_name, state, options=options
        )
