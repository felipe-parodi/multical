...
def extract_pose(points, board, camera, exclude_bad_poses, pose_error_limit):
    ...
    return (
        valid_pose(
            t=rtvec.to_matrix(poses),
            num_points=len(detections.ids),
            error=error,
            angles=list(angles),
        )
...
def map_table(f, point_table, boards, cameras, exclude_bad_poses, pose_error_limit):
    return [
        [
            [
                f(points, board, camera, exclude_bad_poses, pose_error_limit)
...
def make_pose_table(point_table, boards, cameras, exclude_bad_poses, pose_error_limit):
    poses = map_table(
        extract_pose, point_table, boards, cameras, exclude_bad_poses, pose_error_limit
    )
...
def make_point_table(detections, boards):
    ...
    def extract_points(frame_dets):
        points, mask = fill_sparse(num_points, frame_dets.corners, frame_dets.ids)
        return struct(points=points, valid=mask)

    points = [
        [[extract_points(d) for d in frame_dets] for frame_dets in cam_dets]
...
dimensions = struct(camera=0, frame=1, board=2)
...
def pattern_overlaps(table, axis=0):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            row_i, row_j = table._index_select(i, axis=axis), table._index_select(
                j, axis=axis
            )
...
def estimate_transform(table, i, j, axis=0):
    ...
    info(
        f"Estimate transform axis={axis}, pair {(i, j)}, "
        f"inliers {inliers.sum()}/{valid.sum()}"
    )

    info(
        f"RMS (frobius): {err_inlier.frobius:.4f} ({err.frobius:.4f})"
...
def table_info(valid, names):
    ...
    info(
        f"Total: {count_valid(valid)}, cameras: {camera_points}, "
...
def estimate_relative_poses_inv(table, axis=2, hop_penalty=0.9):
    return inverse(
        estimate_relative_poses(inverse(table), axis=axis, hop_penalty=hop_penalty)
    )


def valid(estimates, point_table):
    valid = (
        np.expand_dims(estimates.camera.valid, [1, 2])
...
def multiply_tables(table1, table2):
    ...
    return Table.create(
        poses=table1.poses @ table2.poses, valid=table1.valid & table2.valid
    )
...
def relative_between_n(table1, table2, axis=0, inv=False):
    ...
    relative_poses = [
        f(poses1, poses2)
        for poses1, poses2 in zip(table1._sequence(axis), table2._sequence(axis))
    ]
...
def report_poses(k, init, ref):
    ...
    for i in range(init.shape[0]):
        info(
            f"{k} {i}: frobius {errs.frobius[i]:.4f}, "
...
def initialise_poses(pose_table, camera_poses=None):
    ...
    if camera_poses is not None:
        ...
        camera = Table.create(
            poses=camera_poses, valid=np.ones(camera_poses.shape[0], dtype=bool)
        )
