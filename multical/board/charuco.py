...
class CharucoBoard(Parameters, Board):
    ...
    def board(self):
        ...
        return cv2.aruco.CharucoBoard_create(
            width, height, self.square_length, self.marker_length, aruco_dict
        )
    ...
    def detect(self, image):
        corners, ids, _ = cv2.aruco.detectMarkers(
            image, self.board.dictionary, parameters=aruco_config(self.aruco_params)
        )
        if ids is None:
        ...
        if ids is None or len(ids) < 10:
            return empty_detection  # DLT needs at least 6 points, 10 gives some margin
