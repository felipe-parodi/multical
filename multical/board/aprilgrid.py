...
def import_aprilgrid():
    try:
        ...
        error(err)
        error(
            "aprilgrid support depends on apriltags2-ethz, a pip package (linux only)"
        )
...
class AprilGrid(Parameters, Board):
    def __init__(
        ...
        assert (
            tag_family == "t36h11"
        ), "apriltags2-ethz currently supports only t36h11, remove this once complete!"
    ...
    def grid(self):
        ...
        return AprilGridDetector(
            h, w, self.tag_length, self.tag_spacing, start_id=self.start_id
        )
    ...
    def board(self):
        ...
        aruco_dict = cv2.aruco.getPredefinedDictionary(
            self.aruco_dicts[self.tag_family]
        )
        grid_board = cv2.aruco.GridBoard_create(
            self.size[0], self.size[1], self.tag_length, spacing_length, aruco_dict
        )
    ...
    def size_mm(self):
        ...
        return [int(square_length * n + spacing_length * (n + 1)) for n in self.size]
    ...
    def draw(self, pixels_mm=1, margin_mm=20):
        ...
        def index2coord(x_index, y_index):
            x_coord = int(
                x_index * square_length + spacing_length * (x_index + 1) + margin
            )
            y_coord = int(
                y_index * square_length + spacing_length * (y_index + 1) + margin
            )
        ...
        def marker_x_index_flip(marker):
            for y_index in range(self.size[1]):
                for x_index in range(self.size[0] // 2):
                    ...
                    marker1 = copy(
                        markers[
                            y_coord : y_coord + int(square_length),
                            x_coord_to_change : x_coord_to_change + int(square_length),
                        ]
                    )
                    marker2 = copy(
                        markers[
                            y_coord : y_coord + int(square_length),
                            x_coord : x_coord + int(square_length),
                        ]
                    )
                    markers[
                        y_coord : y_coord + int(square_length),
        ...
        dims = [
            int(square_length * n + spacing_length * (n + 1) + margin * 2)
            for n in self.size
        ]

        markers = self.board.draw(
            tuple(dims),
            marginSize=int(margin + spacing_length),
        ...
        for i in range(self.size[0] + 1):
            for j in range(self.size[1] + 1):
                ...
                cv2.rectangle(
                    markers,
                    (int(x), int(y)),
                    (int(x + spacing_length), int(y + spacing_length)),
                    (0, 0, 0),
                    cv2.FILLED,
                )
    ...
    def detect(self, image):
        ...
        corner_detections = [
            struct(ids=id * 4 + k % 4, corners=corner)
            for k, id, corner in zip(
                range(len(detections.ids)), detections.ids, detections.image_points
            )
        ]

        refined = subpix_corners(
            image, Table.stack(corner_detections), self.subpix_region
        )
        return refined

    def has_min_detections(self, detections):
        tag_ids = detections.ids // 4
        return has_min_detections_grid(
            self.size, tag_ids, min_points=self.min_points, min_rows=self.min_rows
        )
