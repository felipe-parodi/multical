...
def create_dict(name, offset):
    dict_id = name if isinstance(name, int) else getattr(cv2.aruco, f"DICT_{name}")
...
def estimate_pose_points(board, camera, detections):
    ...
    valid, rvec, tvec, error = cv2.solvePnPGeneric(
        objPoints,
        undistorted,
        camera.intrinsic,
        distCoeffs=np.zeros((0,), dtype=np.float32),
    )
