...
def load_images(filenames, prefix=None, **map_options):
    if prefix is not None:
        filenames = [
            [path.join(prefix, file) for file in camera_files]
...
def detect_images(boards, images, j=None, **map_options):
    ...
    print(
        f"Using {camera_threads} camera threads with {threads_per_camera} threads per camera"
    )
...
def intersect_detections(board, d1, d2):
    ...
    if len(ids) > 0:
        return struct(
            points1=d1.corners[inds1],
            points2=d2.corners[inds2],
