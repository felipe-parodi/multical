...
try:
    ...
    print(
        "Please ensure the script is run from a location where the local 'multical' package is discoverable,"
    )
...
output_dirname = (
    os.path.splitext(os.path.basename(pkl_calib_results))[0] + "_outlier_viz"
)
VIZ_OUTPUT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(pkl_calib_results)), output_dirname
)
...
def load_multical_results(filepath: str):
    ...
    try:
                ...
                if all(hasattr(data, attr) for attr in required_attrs):
                    ...
                    if isinstance(data.calibrations, dict) and data.calibrations:
                        ...
                        if isinstance(first_calib, Calibration):
                            ...
                            print(
                                "Error: First item in workspace.calibrations is not a Calibration object."
                            )
                            return None
                    elif (
                        isinstance(data.calibrations, list) and data.calibrations
                    ):  # Handle list case too
                        if isinstance(data.calibrations[0], Calibration):
                            print("Workspace contains valid Calibration object(s).")
                            return data  # Return the whole workspace
                        else:
                            print(
                                "Error: First item in workspace.calibrations list is not a Calibration object."
                            )
                            return None
                    else:
                        print(
                            "Error: workspace.calibrations is not a non-empty dict or list."
                        )
                        return None
                else:
                    print(
                        f"Error: Workspace object is missing one or more required attributes: {required_attrs}"
                    )
                    return None
            else:
        ...
        print(
            f"Error: Could not unpickle data from {filepath}. File might be corrupted or incompatible."
        )
        return None
    except ImportError:
        # Catch potential issues if the pickled object's class isn't found
        print(
            f"Error: Could not import a class required by the pickled object in {filepath}."
        )
        print(
            "Ensure the necessary version of 'multical' is available in the environment."
        )
        return None
...
def get_outlier_info(
    calib: Calibration, camera_names_struct: object, num_outliers: int = 10
):
    ...
    try:
        ...
        print(
            f"Warning: Could not get camera names list from camera_names_struct.camera: {e}"
        )
        camera_names = []  # Fallback

    if not hasattr(calib, "reprojected") or not hasattr(calib, "point_table"):
        print(
            "Error: Calibration object missing 'reprojected' or 'point_table' attributes."
        )
        return []

    try:
        # Calculate errors for ALL points first
        errors_all, valid_all = tables.reprojection_error(
            calib.reprojected, calib.point_table
        )
        print(
            f"Calculated all reprojection errors. Shape: {errors_all.shape}, Total valid points initially: {np.sum(valid_all)}"
        )

        # Check if an inlier mask exists and apply it
        if (
            hasattr(calib, "inliers")
            and isinstance(calib.inliers, np.ndarray)
            and calib.inliers.shape == valid_all.shape
        ):
            print(
                f"Applying final inlier mask. Total inliers in mask: {np.sum(calib.inliers)}"
            )
            final_valid_mask = valid_all & calib.inliers
        else:
            print(
                "Warning: Final inlier mask not found or incompatible. Using only reprojection validity."
            )
        ...
        for i, flat_idx in enumerate(top_n_indices_flat):
            ...
            try:
                ...
                original_indices = tuple(
                    dim_array[flat_idx] for dim_array in valid_indices_tuple
                )
                ...
                cam_name = (
                    ...
                    if camera_names and len(camera_names) > cam_idx
                    else f"Cam_{cam_idx}"
                )
                ...
                if hasattr(calib, "boards") and calib.boards:
                    if (
                        hasattr(calib.boards, "names")
                        and calib.boards.names
                        and len(calib.boards.names) > board_idx
                    ):
                ...
                # Print concise info
                print(
                    f"  {metadata['rank']}. Error: {metadata['error']:.4f} | "
        ...
        print(
            "The structure of the loaded Calibration object might be different than expected."
        )
        return []
...
def visualize_frame_detections(
    ...
    if not (
        ...
        and isinstance(workspace.calibrations, collections.OrderedDict)
    ):
    ...
    try:
        ...
        print(
            "Error: Could not get camera names from workspace.names.camera"
    ...
    for cam_name in cam_names:
        if cam_name not in cam_name_to_idx:
            print(
                f"Warning: Camera '{cam_name}' not found in workspace camera list. Skipping visualization."
            )
            continue
        ...
        try:
            ...
            if (
                isinstance(workspace.filenames, (list, tuple))
                and len(workspace.filenames) > cam_idx
                and isinstance(workspace.filenames[cam_idx], (list, tuple))
                and len(workspace.filenames[cam_idx]) > frame_idx
            ):
                img_filename = workspace.filenames[cam_idx][frame_idx]
                if not isinstance(img_filename, str):
                    print(
                        f"Warning: Found filename for {cam_name}, frame {frame_idx} is not a string ({type(img_filename)}). Skipping."
                    )
            ...
            # elif isinstance(workspace.filenames, dict) ...
                ...
                print(
                    f"Warning: workspace.filenames structure not recognized or index out of bounds for {cam_name}, frame {frame_idx}. Attempting fallback naming."
                )
                # Fallback: Assume standard naming like frame_0XXX.png/jpg in cam folder
                cam_image_dir = os.path.join(image_base_dir, cam_name)
                potential_png = os.path.join(
                    cam_image_dir, f"frame_{frame_idx:04d}.png"
                )
                potential_jpg = os.path.join(
                    cam_image_dir, f"frame_{frame_idx:04d}.jpg"
                )
                if os.path.exists(potential_png):
                    ...
                    print(
                        f"Error: Could not find image file for {cam_name}, frame {frame_idx} using fallback naming in {cam_image_dir}"
                    )
                    continue  # Skip this camera
        except Exception as e:
            print(
                f"Error accessing workspace.filenames for {cam_name}, frame {frame_idx}: {e}. Attempting fallback."
            )
            ...
            if os.path.exists(potential_png):
                ...
                print(
                    f"Error: Could not find image file for {cam_name}, frame {frame_idx} using fallback naming after error."
                )
                continue

        if not img_filename or not os.path.exists(img_filename):
            print(
                f"Error: Image file path '{img_filename}' not found or invalid for {cam_name}, frame {frame_idx}. Skipping."
            )
            continue
        ...
        try:
            ...
            print(
                f"    Inspecting workspace.point_table: type={type(workspace.point_table)}"
            )
            if hasattr(workspace.point_table, "__dict__"):
                print(
                    f"      point_table attributes: {list(workspace.point_table.__dict__.keys())}"
                )
            if hasattr(workspace.point_table, "points"):
                print(
                    f"      point_table.points: type={type(workspace.point_table.points)}"
                )
            if hasattr(workspace.point_table, "valid"):
                print(
                    f"      point_table.valid: type={type(workspace.point_table.valid)}"
                )

            print(
                f"    Inspecting calib_obj.reprojected: type={type(calib_obj.reprojected)}"
            )
            if hasattr(calib_obj.reprojected, "__dict__"):
                print(
                    f"      reprojected attributes: {list(calib_obj.reprojected.__dict__.keys())}"
                )
            # <<< End Inspection >>>
            ...
            uv_detected = workspace.point_table.points[
                cam_idx, frame_idx, board_idx, :, :
            ]
            # Try accessing reprojected points via a .points attribute, similar to point_table
            if hasattr(calib_obj.reprojected, "points") and isinstance(
                calib_obj.reprojected.points, np.ndarray
            ):
                uv_reprojected = calib_obj.reprojected.points[
                    cam_idx, frame_idx, board_idx, :, :
                ]
            else:
                # Fallback/Error - If .points doesn't exist or isn't an array
                print(
                    f"Error: Cannot find suitable numpy array in calib_obj.reprojected for {cam_name}, frame {frame_idx}. Skipping point drawing for this camera."
                )
            ...
            print(
                f"Error accessing point data for {cam_name}, frame {frame_idx}: {e}. Skipping."
            )
            continue
        ...
        for pt_idx in range(len(valid_mask)):
            if valid_mask[pt_idx]:
                ...
                cv2.drawMarker(
                    img_vis, center_rep, (0, 0, 255), cv2.MARKER_CROSS, 10, 1
                )  # Red
...
if __name__ == "__main__":
    ...
    if workspace:
        ...
        print(
            f"Inspecting workspace.names: type={type(workspace.names)}, value={workspace.names}"
        )
        # <<< End Inspection >>>
        ...
        if isinstance(workspace.calibrations, dict) and workspace.calibrations:
            calib_obj = next(iter(workspace.calibrations.values()))
            print(
                f"Extracted Calibration object (type: {type(calib_obj)}). Proceeding with analysis."
            )
        elif (
            isinstance(workspace.calibrations, list) and workspace.calibrations
        ):  # Handle list case
            calib_obj = workspace.calibrations[0]
            print(
                f"Extracted Calibration object from list (type: {type(calib_obj)}). Proceeding with analysis."
            )
        else:
            print(
                "Error: Could not extract a Calibration object from workspace.calibrations."
            )
            calib_obj = None

        if calib_obj and isinstance(calib_obj, Calibration):
            # Pass workspace.names to the function
            outliers = get_outlier_info(
                calib_obj, workspace.names, num_outliers=N_TOP_OUTLIERS
            )
            ...
            if outliers:  # Only proceed if we have outliers
                ...
                sorted_frames_by_count = sorted(
                    frame_counts.items(), key=lambda item: item[1], reverse=True
                )
                ...
                sorted_cams_by_count = sorted(
                    cam_counts.items(), key=lambda item: item[1], reverse=True
                )
                ...
                try:
                    all_errors, all_valid = tables.reprojection_error(
                        calib_obj.reprojected, workspace.point_table
                    )
                    frame_max_errors = {}
                    ...
                    if valid_indices_tuple[0].size > 0:
                        ...
                        for frame_idx_iter in unique_frames:
                            errors_for_frame = valid_error_values[
                                frame_indices == frame_idx_iter
                            ]
                            if errors_for_frame.size > 0:
                                frame_max_errors[frame_idx_iter] = np.max(
                                    errors_for_frame
                                )
                            else:
                                frame_max_errors[frame_idx_iter] = 0

                        sorted_frame_errors = sorted(
                            frame_max_errors.items(),
                            key=lambda item: item[1],
                            reverse=True,
                        )

                        print(
                            f"--- Top {min(N_TOP_FRAMES, len(sorted_frame_errors))} Frames by Max Reprojection Error ---"
                        )
                        for i in range(min(N_TOP_FRAMES, len(sorted_frame_errors))):
                            frame_idx_iter, max_err = sorted_frame_errors[i]
                            print(
                                f"  {i+1}. Frame {frame_idx_iter}: Max Error = {max_err:.4f}"
                            )
                            top_frames_to_visualize.append(
                                frame_idx_iter
                            )  # Add frame to visualization list
                    else:
                ...
                if top_frames_to_visualize:
                    if IMAGE_BASE_DIR and os.path.isdir(IMAGE_BASE_DIR):
                        ...
                        if "all_errors" not in locals() or "all_valid" not in locals():
                            print(
                                "Error: Full error data not available for dynamic camera selection."
                            )
                        else:
                            ...
                            if (
                                "valid_indices_tuple" not in locals()
                                or "valid_error_values" not in locals()
                            ):
                                print(
                                    "Error: Required index/error variables not in scope for dynamic visualization."
                                )
                            else:
                                valid_cam_indices = valid_indices_tuple[
                                    0
                                ]  # Direct assignment (already filtered by where)
                                valid_frame_indices = valid_indices_tuple[
                                    1
                                ]  # Direct assignment (already filtered by where)
                                ...
                                for frame_to_viz in top_frames_to_visualize:
                                    ...
                                    if errors_in_frame.size > 0:
                                        ...
                                        sorted_error_indices_in_frame = np.argsort(
                                            errors_in_frame
                                        )[::-1]
                                        # Get camera indices corresponding to top N errors
                                        top_cam_indices = cams_in_frame[
                                            sorted_error_indices_in_frame[
                                                :N_TOP_CAMERAS_PER_FRAME
                                            ]
                                        ]
                                        unique_top_cam_indices = np.unique(
                                            top_cam_indices
                                        )
                                        # Convert indices to names
                                        try:
                                            cameras_to_visualize_for_frame = [
                                                workspace.names.camera[idx]
                                                for idx in unique_top_cam_indices
                                            ]
                                            print(
                                                f"  Selected cameras for Frame {frame_to_viz} (Top {N_TOP_CAMERAS_PER_FRAME} errors): {cameras_to_visualize_for_frame}"
                                            )
                                        except (
                                            AttributeError,
                                            IndexError,
                                            ...
                                            print(
                                                f"  Error converting top camera indices to names for Frame {frame_to_viz}: {e}"
                                            )
                                            cameras_to_visualize_for_frame = (
                                                []
                                        ...
                                        print(
                                            f"  No valid errors found in Frame {frame_to_viz} to select cameras."
                                        )

                                    # Call visualization if we have cameras selected
                                    if cameras_to_visualize_for_frame:
                                        visualize_frame_detections(
                                            workspace,
                                            frame_to_viz,
                                            cameras_to_visualize_for_frame,
                                            IMAGE_BASE_DIR,
                                        ...
                                        print(
                                            f"  Skipping visualization for Frame {frame_to_viz} due to lack of selected cameras."
                                        )
                    else:
                        print(
